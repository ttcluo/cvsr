...
class VideoTestDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        if opt['name'].lower() in ['vid4', 'reds4', 'redsofficial','yt4']:
...
@DATASET_REGISTRY.register()
class VideoRecurrentTestDatasetShortREDS(VideoTestDataset):

    def __init__(self, opt):
        super(VideoRecurrentTestDatasetShortREDS, self).__init__(opt)
        # Find unique folder strings
        self.folders = sorted(list(set(self.data_info['folder'])))
    ...
    def __getitem__(self, index):
        ...
        if self.cache_data:
            imgs_lq = self.imgs_lq[folder][0:9]
            imgs_gt = self.imgs_gt[folder][0:9]
        else:
            raise NotImplementedError('Without cache_data is not implemented.')
