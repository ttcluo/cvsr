...
class CLSTM(nn.Module):
    ...
    def __init__(self, input_chans, filter_size,growthrate, num_layers):
        ...
        for complexblock in range(0, self.num_layers):
            block =  ResidualBlockNoBN(16, 16)
            complex_list.append(block)
            compress_list.append(nn.Conv2d(32, 16, 1, 1, 0))
            conv_list1.append(nn.Conv2d(16,16,3,1,1))
            conv_list2.append(nn.Conv2d(16, 16, 3, 1, 1))
            list_3dconv.append(nn.Sequential(nn.Conv2d(32,16,(3,3),(1,1),(1,1)),
                                    nn.ReLU(),
        ...
        ##########
        # self.convreal = nn.Conv2d(16,16,3,1,1)
        # self.convimg = nn.Conv2d(16, 16, 3, 1, 1)
        ###########
        self.cell_list = nn.ModuleList(cell_list)
        self.Invcell_list = nn.ModuleList(Invcell_list)
        self.complex_list = nn.ModuleList(complex_list)
        self.compress_list = nn.ModuleList(compress_list)
        self.conv_list1 = nn.ModuleList(conv_list1)
        self.conv_list2 = nn.ModuleList(conv_list2)
        self.list_3dconv = nn.ModuleList(list_3dconv)
    ...
    def forward(self, input):
        ...
        for idlayer in range(self.num_layers):  # loop for every layer
            ...
            compress_func = self.compress_list[idlayer]
            resblock1 = self.complex_list[idlayer]
            conv3D = self.list_3dconv[idlayer]
            convreal = self.conv_list1[idlayer]
            convimg = self.conv_list2[idlayer]
            ...
            for h in range(len(output_inner_f)):
                ...
                real = v1 - v2
                img = resblock1(compress_func(torch.cat([v1,v2],dim=1)))
                sreal = real
                simg = img
                newrep = torch.cat([sreal, simg], dim=1)
                phi = conv3D(newrep)
                real = real * torch.cos(phi)
                img = img * torch.sin(phi)
                v1 = real + convreal(real)
                v2 = img + convimg(img)
...
if __name__ == '__main__':
    ...
    conv_lstm = CLSTM(inp_chans, filter_size, num_features, nlayers)
