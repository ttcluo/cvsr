...
class VideoRecurrentModelComplex(VideoBaseModel):

    def __init__(self, opt):
        super(VideoRecurrentModelComplex, self).__init__(opt)
        if self.is_train:
    ...
    def optimize_parameters(self, current_iter):
        ...
        self.phaserot = (torch.rand((batch,1,1,1)).to(self.device)*np.pi/2)-np.pi/4
